const convertNumberToWordsInNepaliSystem = (
  num: number | string,
  local?: 'np' | 'en'
): string => {
...
const handleNumberString = (num: string, local?: 'np' | 'en') => {
  const zeroTo99 = [];
  const zeroTo9 = [
    'Zero',
    'One',
...
const convertFirstThreeDigitsToWordsInNepali = (
  numberStr: string,
  zeroTo9: string[]
): string => {
...
const convert = (x: string, zeroTo99: string[]): string => {
...
if (typeof window !== 'undefined') {
  (window as any).nepaliNumberSystem = {
